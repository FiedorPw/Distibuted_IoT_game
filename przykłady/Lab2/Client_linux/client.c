...
#define BLINK 'b'   // blink - żądanie miganie diodą
#define BLINK_ACK 'a' // blink - potwierdzenie mignięcia diodą
...
void send_blink(int server_socket, struct sockaddr_in client_addr, socklen_t client_len) {
    char blink_cmd = BLINK;
    printf("\033[34mWysyłanie komendy BLINK [%c]\033[0m\n", BLINK);
    ...
    if (recv_len > 0) {
        buffer[recv_len] = '\0';
        if (buffer[0] == BLINK_ACK) {
            printf("\033[32mOtrzymano potwierdzenie BLINK\033[0m\n");
        } else {
            printf("\033[31mOtrzymano nieprawidłową odpowiedź na BLINK\033[0m\n");
        }
    }
}
...
void client_listen(int server_socket, struct sockaddr_in sender_addr, socklen_t client_len) {
    ...
    if (recv_len > 0) {
        ...
        switch(header) {
            case BLINK:
                    printf("\033[32mOtrzymano komendę BLINK\033[0m\n");
                    break;
                case BLINK_ACK:
                    printf("\033[32mOtrzymano potwierdzenie BLINK\033[0m\n");
                    break;
